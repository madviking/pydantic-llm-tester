...
1. **Provider Configurations**: `src/pydantic_llm_tester/llms/<provider_name>/config.json` - Define provider-specific settings like API key environment variable and system prompt. Model details are now managed centrally or fetched dynamically (especially for OpenRouter).
...
Model details (like cost, token limits, and enabled status) are now managed centrally by the framework, often fetched dynamically from providers like OpenRouter, and can be influenced by the `pyllm_config.json` file. The `llm_models` array is no longer present in provider `config.json` files.
...
**1. `run`**
   ...
   - Options:
     ...
     - `--models <provider:model> [<provider:model>...]` or `-m <provider:model>`: Specify exact LLM models to use in the format `provider_name:model_name` (e.g., `openai:gpt-4o`, `openrouter:mistral-large-latest`). Can be used multiple times.
...
**2. `list`**
   ...
   - Options:
     ...
     - `--models <provider:model> [<provider:model>...]` or `-m <provider:model>`: Specify LLM models to consider for provider listing in the format `provider_name:model_name`.
...
2. **API Keys**: Use the `llm-tester configure keys` command or manually edit the `.env` file.
3. **Provider Settings**: Manually edit the specific `src/pydantic_llm_tester/llms/<provider_name>/config.json` file for things like the default system prompt. Model details are managed centrally.
4. **Directly edit files**: Modify the JSON files (`pyllm_config.json`, provider `config.json`) directly if needed.

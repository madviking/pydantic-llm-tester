...
1.  **Create OpenRouter API Client Class:**
    *   Location: `src/llm_tester/infrastructure/openrouter/client.py`
    ...
    import logging
    from llm_tester.core.exceptions import APIConnectionError # Adjusted import
...
1.  **Create `CostInformationService`:**
    *   Location: `src/llm_tester/services/cost_service.py`
    ...
    from llm_tester.infrastructure.openrouter.client import OpenRouterClient
    from llm_tester.infrastructure.openrouter.schemas import OpenRouterModel
    ...
    from llm_tester.core.exceptions import CostInfoNotAvailable, ConfigurationError
...
1.  **Create/Update `CostCalculator`:**
    ...
    *   This replaces the `CostCalculator` from `src/utils/cost_manager.py`.
    ...
    import logging
    from typing import Dict, Any, Optional, List
    from llm_tester.services.cost_service import CostInformationService # Adjusted import
    from llm_tester.core.exceptions import CostInfoNotAvailable
...
2.  **Create `UsageTracker` Service:**
    ...
    *   **Persistence:** For a CLI tool, in-memory tracking per session is primary. Optionally, allow logging usage to a file (e.g., JSONL format) for later analysis.
    ...
    from llm_tester.services.cost_calculator import CostCalculator # Adjusted import
...
1.  **Create `CostAPI` Facade:**
    *   Location: `src/llm_tester/cost_api.py` (or `src/llm_tester/services/cost_management_api.py`)
    ...
    from llm_tester.infrastructure.openrouter.client import OpenRouterClient
    from llm_tester.services.cost_service import CostInformationService
    from llm_tester.services.cost_calculator import CostCalculator
    from llm_tester.services.token_counter import TokenCounter
    from llm_tester.services.usage_tracker import UsageTracker, UsageEntry
    from llm_tester.infrastructure.openrouter.schemas import OpenRouterModel
    from llm_tester.core.exceptions import ConfigurationError

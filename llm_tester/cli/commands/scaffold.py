...
app = typer.Typer(help="Commands for scaffolding new providers and models.")

# Determine the directory containing the templates relative to the package root
_current_file_dir = os.path.dirname(os.path.abspath(__file__))
_cli_dir = os.path.dirname(_current_file_dir) # Go up one level to llm_tester/cli
_llm_tester_dir = os.path.dirname(_cli_dir) # Go up another level to llm_tester
_templates_dir = os.path.join(_llm_tester_dir, "cli", "templates") # Path is llm_tester/cli/templates
...
def scaffold_provider(
    ...
    providers_dir: Optional[str] = typer.Option(None, "--providers-dir", help="Directory to create the provider in (defaults to llm_tester/llms)."),
    ...
    if interactive:
        ...
        # providers_dir = typer.prompt("Enter the directory to create the provider in (default: llm_tester/llms)", default="llm_tester/llms")
    ...
    if providers_dir:
        ...
        # Default to llm_tester/llms relative to the package directory
...
def scaffold_model(
    ...
    models_dir: Optional[str] = typer.Option(None, "--models-dir", help="Directory to create the model in (defaults to ./models)."),
    interactive: bool = typer.Option(False, "--interactive", "-i", help="Enable interactive mode for scaffolding.")
):
    """
    ...
    if interactive:
        ...
        # models_dir = typer.prompt("Enter the directory to create the model in (default: ./models)", default="./models")
    elif not model_name:
        typer.echo("Error: Model name must be provided in non-interactive mode.", err=True)
        raise typer.Exit(code=1)

    # Determine the base directory for models
    base_dir = models_dir or "./models" # Default to a 'models' directory in the current working directory
